...
public class Sarsa extends TDAlgorithm<float[]> implements
	...
	public Sarsa(Function q, float learningRate, float epsilonGreedy,
	...
	public void selectAction(int action) {
		...
		if (this.previousState != null && this.learning) {
			float value = this.actionValue * this.discountRate + this.reward;
	...
	public boolean saveKnowledge(String filename) throws IOException {
		...
		try {
			...
			stream.writeFloat(this.learningRate);
			stream.writeFloat(this.epsilonGreedy);
			stream.writeFloat(this.discountRate);
			stream.writeFloat(this.rewardScale);
			stream.writeFloat(this.minReward);
			stream.writeFloat(this.maxReward);
			stream.writeBoolean(this.learning);
			return true;
		} finally {
			stream.close();
		}
	...
	public boolean loadKnowledge(String filename) throws IOException {
		...
		try {
			...
			this.learningRate = stream.readFloat();
			this.epsilonGreedy = stream.readFloat();
			this.discountRate = stream.readFloat();
			this.rewardScale = stream.readFloat();
			this.minReward = stream.readFloat();
			this.maxReward = stream.readFloat();
			this.learning = stream.readBoolean();
			return true;
